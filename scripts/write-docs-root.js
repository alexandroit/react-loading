...
function buildCards() {
  return releaseLines.map((line) => {
    ...
    return [
      ...
      '          </div>',
      `          <h2>${line.label}</h2>`,
      `          <p>Maintained React wrapper docs for the ${line.runtime} runtime line, keeping the same ReviveJS loading engine with the React conventions expected by that generation.</p>`,
...
fs.writeFileSync(
  ...
  [
    ...
    '    <title>@revivejs/react-loading</title>',
    '    <style>',
    '      :root {',
    '        color-scheme: light;',
    ...
    '    <main>',
    '      <section class="hero">',
    '        <span class="eyebrow">ReviveJS React loading wrapper</span>',
    '        <h1>@revivejs/react-loading</h1>',
    '        <p>Versioned React bindings for the ReviveJS loading toolkit. Each maintained line keeps the same loader engine and public ideas while fitting the runtime expectations of React 17, 18, and 19.</p>',
    '        <div class="hero-actions">',
    '          <a class="hero-link primary" href="./react-19/">Open React 19 docs</a>',
    '          <a class="hero-link secondary" href="https://www.npmjs.com/package/@revivejs/react-loading">npm package</a>',
    '        </div>',
    '      </section>',
